...
function buildPeopleIndexHtml({ people }) {
  ...
  const rolePills = [
    `<button class="role-pill small" data-role="__all__" aria-pressed="true" type="button">全部</button>`,
    ...roleTags.map(r => `<button class="role-pill small" data-role="${esc(r)}" aria-pressed="false" type="button">${esc(r)}</button>`)
  ].join("");
...
<head>
  ...
  <style>
    ...
    .role-filters{margin-top:.75rem;display:flex;flex-wrap:wrap;gap:.5rem;align-items:center;}
    .role-pill{
    ...
    .role-pill[aria-pressed="true"]{border-color:rgba(59,130,246,.7);color:#1d4ed8;}
    .role-pill.small{font-size:.9rem;padding:.35rem .6rem;}
    ul.grid{margin-top:1.25rem;list-style:none;padding:0;display:grid;grid-template-columns:repeat(1,minmax(0,1fr));gap:.75rem;}
    ...
    .page-footer{margin-top:32px;padding-top:24px;border-top:1px solid rgba(148,163,184,.35);color:#64748b;font-size:14px;}
    .page-footer a{color:#1d4ed8;text-decoration:none;}
    .page-footer a:hover{text-decoration:underline;}
  </style>
</head>

<body class="page people-index-page">
  <div id="global-header-wrapper"></div>

  <main class="wrap">
    <header>
      <h1>论语人物索引</h1>
      <div class="sub">默认按关联条目数排序。支持按身份标签筛选与名称检索。</div>
      <p class="intro">本页汇总《论语》相关人物。你可以从高频人物开始学习。也可以通过筛选快速定位。进入人物详情页即可查看简介与相关条目。</p>
      ...
      <div class="role-filters" aria-label="身份标签筛选" id="role-filters">
        <span class="sub">筛选</span>
        ${rolePills}
      </div>
...
${items}
    ...
    <footer class="page-footer">
      <div>© <span id="year"></span> Lunyu Index. <a href="/index.html">首页</a>. <a href="/chapters.html">二十篇全文</a>.</div>
    </footer>
  </main>
  ...
  <script>
    ...
    function setRolePressed(){
      ...
      roleFilters.querySelectorAll('.role-pill').forEach(btn=>{
        btn.setAttribute('aria-pressed', btn.dataset.role === roleMode ? 'true' : 'false');
      });
    }
    ...
    function apply(){
      ...
      const filtered = items.filter(li=>{
        ...
        const role = (li.dataset.role || '').trim();
        const roleOk = (roleMode === '__all__') || (role === roleMode);
...
function buildPersonHtml({ person }) {
  ...
  const desc = person.description?.trim() || `${person.name}在《论语》中的相关条目与学习要点。`;
...
<head>
  ...
  <script type="application/ld+json">${esc(JSON.stringify(ldJson))}</script>
  <style>
    .page{min-height:100vh;}
    .wrap{max-width:980px;margin:0 auto;padding:24px 16px;}
    .breadcrumb{display:flex;gap:.35rem;flex-wrap:wrap;align-items:center;font-size:14px;color:#64748b;}
    .breadcrumb a{color:#1d4ed8;text-decoration:none;}
    .breadcrumb a:hover{text-decoration:underline;}
    h1{font-size:2.2rem;font-weight:900;line-height:1.1;margin:.6rem 0 0 0;color:#0f172a;}
    .hero{margin-top:8px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;}
    .tag{display:inline-flex;align-items:center;font-size:.9rem;border:1px solid rgba(148,163,184,.5);
      border-radius:999px;padding:.25rem .6rem;background:#fff;color:#475569;}
    .intro{margin-top:1rem;line-height:1.85;color:#334155;max-width:820px;}
    .links{margin-top:1.15rem;display:flex;gap:.75rem;flex-wrap:wrap;}
    .links a{color:#1d4ed8;text-decoration:none;}
    .links a:hover{text-decoration:underline;}
    .block{margin-top:1.4rem;border:1px solid rgba(148,163,184,.35);border-radius:1rem;background:rgba(255,255,255,.92);padding:1rem;}
    .block h2{margin:0 0 .25rem 0;font-size:1.1rem;color:#0f172a;}
    .muted{color:#64748b;line-height:1.7;}
    /* 人物页：把高级筛选折叠成“人物已锁定 + 关键词” */
    body.person-page .analects-search-header{display:none;}
    body.person-page .analects-advanced-filters{display:none;}
    body.person-page #analects-reset-btn{display:none;}
    body.person-page .analects-search-actions{justify-content:flex-start;}
    body.person-page .analects-search-actions .analects-btn{width:auto;}
    body.person-page .analects-keyword-label{font-weight:700;}
    body.person-page .analects-selected-items{display:block !important;margin-top:10px;}
    .page-footer{margin-top:32px;padding-top:24px;border-top:1px solid rgba(148,163,184,.35);color:#64748b;font-size:14px;}
    .page-footer a{color:#1d4ed8;text-decoration:none;}
    .page-footer a:hover{text-decoration:underline;}
  </style>
...
<body class="page person-page">
  ...
  <main class="wrap">
    ...
    <header>
      ...
      <div class="hero">
        <span class="tag">${esc(person.role_tag)}</span>
      </div>
    ...
    <section class="block" aria-label="相关条目">
      <h2>与 ${esc(person.name)} 相关的论语条目</h2>
      <p class="muted">已自动锁定人物条件。你可以输入关键词进一步缩小范围。</p>
  ...
  <script type="module">
    ...
    try {
      ...
      sdk.selectedItems.characters.set(PERSON_ID, { id: PERSON_ID, name: PERSON_NAME });
