...
// [稳定最终版] 结合了初始加载修复和焦点事件修复
  async _initializeSession() {
    
    // --- 步骤 1: [关键] 立即获取并应用初始 Session ---
    // 这是确保页面首次加载时，所有API请求都能成功的核心。
    ...
    // --- 步骤 2: 注册 Auth 状态变化监听器 ---
    // 处理登录、登出、Token刷新等事件。
    this.supabase.auth.onAuthStateChange(async (event, session) => {
      // 仅在 session 状态实际变化时进行复杂操作
      ...
      if (isLoggedIn !== wasLoggedIn || event === 'TOKEN_REFRESHED') {
        if (session) {
