...
class AnalectsSDK {
  ...
  // 5. [最终版本] 创建一个新的私有方法来处理会话初始化
  async _initializeSession() {
    // Supabase-js v2 的标准用法，获取当前会话
    ...
    if (session) {
	  ...
	  await this._loadUserFavorites(); // [新增] 初始加载时获取收藏
	  this._updateFavoriteButtonsUI(); // [新增] 初始加载后刷新UI
    } else {
      this.currentUser = null;
      // 保持 header 为默认的 anon key
    }

    // 监听后续的认证状态变化 (例如用户登录、登出)
    this.supabase.auth.onAuthStateChange(async (event, session) => {
      if (session) {
        // 用户登录了
        this.currentUser = session.user;
        this.headers.Authorization = `Bearer ${session.access_token}`;
		await this._loadUserFavorites(); // [新增] 登录时获取收藏
      } else {
        // 用户登出了
        this.currentUser = null;
        // 恢复为公共的 anon key
        this.headers.Authorization = `Bearer ${this.supabaseKey}`; 
		this.favoriteIds.clear(); // [新增] 登出时清空收藏
      }
      // [新增] 无论登录还是登出，都在状态变化后刷新按钮UI
      this._updateFavoriteButtonsUI();
	  
      // --- [最终修正版] 检查登录事件，并显示 Toast ---
      
      // 'SIGNED_IN' 用于Email登录, 'INITIAL_SESSION' 用于Google等第三方登录返回
      const isLoginEvent = (event === 'SIGNED_IN' || event === 'INITIAL_SESSION');
      
      // 使用 sessionStorage 来防止刷新页面时重复显示 Toast
      const hasShownToast = sessionStorage.getItem('loginToastShown');

      // [核心修改] 只有当是登录事件、有真实用户会话、且提示未显示过时，才显示Toast
      if (isLoginEvent && session && !hasShownToast) {
        this.closeAuthModal();
        if (window.showToast) {
          window.showToast('登录成功，欢迎回来！');
        }
        // 设置一个标志，表示在这个浏览器会话中已经显示过欢迎提示了
      ...
      console.log('用户状态发生变化:', this.currentUser, '事件是:', event);
      
      // [新增] 重新渲染全局小部件以反映状态变化
      // 检查全局小部件的容器是否存在，如果存在，则重新渲染它
  ...
  // [修正版] 生成结果卡片HTML （支持显示收藏时间）
  generateResultCardHTML(entry) {
    if (!entry) return ''; // 安全检查
    ...
    return `
	  ...
	  ${entry.favorited_at ? `
		        <div class="analects-card-footer">
		          <span class="footer-text">收藏于 ${new Date(entry.favorited_at).toLocaleDateString()}</span>
		        </div>
		      ` : ''}
    `;
  }
  ...
  // [新增] 获取当前用户的收藏列表（包含收藏时间）
  async getMyFavorites() {
    // 1. 检查用户是否登录，如果未登录则无法获取收藏，直接返回空数组
    if (!this.currentUser) {
    ...
    try {
      // 2. 查询 user_favorites 表，请求 created_at 字段
      const { data, error } = await this.supabase
        .from('user_favorites')
        .select(`
          created_at, 
          analects_entries_expanded (
            *
          )
        `)
      ...
      // 3. 处理返回的数据，提取出完整的论语条目信息
      // Supabase 返回的数据结构是 [{ entry_id: ..., analects_entries_expanded: { ... } }]
      // 我们将其转换为一个更简洁的数组：[{...}, {...}]
      // [核心修改点] 在处理数据时，将 created_at 附加到最终的对象上
      const favorites = data.map(item => {
        if (!item.analects_entries_expanded) return null;
        
        // 将 analects_entries_expanded 对象和收藏时间 created_at 合并
        return {
          ...item.analects_entries_expanded,
          favorited_at: item.created_at // 创建一个新属性 favorited_at
        };
      }).filter(Boolean);
  ...
  async renderMyFavoritesPage(container) {
    ...
    container.innerHTML = '<div class="analects-loading text-center py-8">正在加载您的收藏...</div>';
  ...
  renderGlobalWidget(container) {
    ...
    if (this.currentUser) {
      // --- 已登录状态 ---
      const userInitial = this.currentUser.email ? this.currentUser.email[0].toUpperCase() : '我';
      widgetHTML = `
        <div class="fab-menu">
          <a href="/my-favorites.html" class="fab-menu-item">我的收藏</a>
          <button id="global-logout-btn" class="fab-menu-item">登出</button>
        </div>
        
        <button class="fab-main-button logged-in" title="打开用户菜单">
          <span>${this.escapeHtml(userInitial)}</span>
          <svg class="fab-chevron" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="m18 15-6-6-6 6"/></svg>
        </button>
      `;
    } else {
      // --- 未登录状态 (保持不变) ---
      widgetHTML = `
        <button id="global-login-btn" class="fab-main-button">登录</button>
      `;
    }
  ...
  _attachGlobalWidgetEvents() {
    const mainBtn = this.widgetContainer.querySelector('.fab-main-button');
    const menu = this.widgetContainer.querySelector('.fab-menu');
    const loginBtn = document.getElementById('global-login-btn');
    const logoutBtn = document.getElementById('global-logout-btn');

    if (loginBtn) {
      // 我们将在下一步实现 showAuthModal
    ...
    if (mainBtn && mainBtn.classList.contains('logged-in')) {
      ...
      if (form) {
        form.addEventListener('submit', async (e) => {
          ...
          if (result.error) {
            ...
            if (view === 'signup') {
              //successDiv.textContent = '注册成功！请检查您的邮箱以完成验证。';
              //form.reset();
              // 不再是显示一行小字，而是渲染整个成功视图
    ...
    // [新增] 加载当前用户的所有收藏ID到内存中
    async _loadUserFavorites() {
      if (!this.currentUser) return; // 确保用户已登录

      try {
        // 我们只需要 entry_id 字段，不需要完整的条目信息
        const { data, error } = await this.supabase
          .from('user_favorites')
          .select('entry_id')
          .eq('user_id', this.currentUser.id)
		  .limit(5000); // [核心修改] 添加一个足够大的 limit，确保能获取所有收藏

        if (error) throw error;

        // 清空旧的 Set，然后将获取到的所有ID添加进去
        this.favoriteIds.clear();
        data.forEach(item => this.favoriteIds.add(item.entry_id));
    ...
    async toggleFavorite(entryId, targetButton) {
      ...
      try {
        ...
        if (window.showToast) {
          window.showToast('操作失败，请稍后重试', true);
        }
      } finally {
