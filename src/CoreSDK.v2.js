...
export default class CoreSDK {
  ...
  renderSearchInterface(container, options = {}) {
    if (!container) {
      console.error('未找到容器元素');
      return;
    }

    // 默认保持兼容: 不传 options 时. 仍渲染完整版本
    this.searchRenderOptions = {
      mode: options.mode || 'full', // 'compact' | 'full'
      // 预留更多配置: 例如是否显示标题等
      ...options
    };

    container.innerHTML = this.getSearchInterfaceHTML(this.searchRenderOptions);
  ...
  // 获取搜索界面HTML
  getSearchInterfaceHTML(options = {}) {
    const mode = options.mode || 'full';
    const modeClass = mode === 'compact' ? 'analects-search--compact' : 'analects-search--full';
    return `
      <div class="analects-search ${modeClass}" data-analects-mode="${mode}">
  ...
  // 渲染每日论语组件
  async renderDailyAnalect(container, options = {}) {
    if (!container) {
      console.error('未找到容器元素');
      return;
    }

    this.dailyRenderOptions = {
      mode: options.mode || 'widget', // 'editorial' | 'widget'
      ...options
    };

    container.innerHTML = `<div class="daily-analect-skeleton"><div class="skeleton-block" style="height: 30px; width: 40%; margin: 0 auto 16px;"></div><div class="skeleton-block" style="height: 20px; width: 60%; margin: 0 auto 48px;"></div><div class="skeleton-block" style="height: 80px; width: 100%; margin-bottom: 16px;"></div><div class="skeleton-block" style="height: 50px; width: 80%; margin: 0 auto;"></div></div>`;
    ...
    try {
      ...
      if (entry) {
        ...
        const shareLinks = this.generateShareLinks(entry);
        container.innerHTML = this.getDailyAnalectHTML(entry, dateInfo, shareLinks, this.dailyRenderOptions);
      } else {
  ...
  // [修正版] 获取每日论语HTML
  getDailyAnalectHTML(entry, dateInfo, shareLinks, options = {}) {
    const mode = options.mode || 'widget';
    const modeClass = mode === 'editorial' ? 'analects-daily--editorial' : 'analects-daily--widget';

    // [核心修改] 在精简版中，isFavorited永远为false
    const isFavorited = false;

    return `
      <div class="analects-daily ${modeClass}" data-analects-mode="${mode}">
