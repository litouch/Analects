...
class AnalectsSDK {
  ...
  // 5. [最终终极版] 初始化会话并保证切换页面不丢登录状态
  async _initializeSession() {
    // --- [新增] 启用自动刷新，确保 token 不会过期 ---
    this.supabase.auth.startAutoRefresh();

    // --- [步骤1] 初始化时先尝试加载现有 session ---
    ...
    if (session) {
      ...
      console.log('初始化时已加载现有 session');
    } else {
      this.currentUser = null;
      console.log('初始化时没有找到 session');
    }
    ...
    this.supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      this._updateFavoriteButtonsUI();   // [新增] 刷新收藏按钮UI
    ...
    // --- [步骤3] 切回页面时尝试从 localStorage 恢复 session ---
    const recoverSession = async () => {
      ...
      if (session && !this.currentUser) {
        ...
        console.log('切回页面后已从 localStorage 恢复 session');
      }
    };
  ...
  renderDailyAnalect(container) {
    if (!container) {
      ...
      <div class="daily-analect-skeleton">
        <div class="skeleton-block" style="height: 30px; width: 40%; margin: 0 auto 16px;"></div>
        <div class="skeleton-block" style="height: 20px; width: 60%; margin: 0 auto 48px;"></div>
        <div class="skeleton-block" style="height: 80px; width: 100%; margin-bottom: 24px;"></div>
        <div class="skeleton-block" style="height: 50px; width: 80%; margin: 0 auto;"></div>
      </div>
    `;
...
// [最终修正版] 渲染“我的收藏”页面
  async renderMyFavoritesPage(container) {
    ...
    `;
    // 循环创建3个骨架卡片，让加载效果更真实
    container.innerHTML = Array(3).fill(skeletonCardHTML).join('');
